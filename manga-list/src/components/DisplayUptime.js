...
class DisplayUptime extends React.Component {
  ...
  updateUptime = () => {
    ...
    this.setState(prevState => ({ displayTime: this.setDateAndTime(prevState.uptime) }))
  }

  setDateAndTime = time => {
    let seconds = time%60, secondsWord;
    (seconds > 1) || (seconds === 0) ? secondsWord = "seconds" : secondsWord = "second"

    let minutes = parseInt(time/60%60), minutesWord;
    (minutes > 1) || (minutes === 0) ? minutesWord = "minutes" : minutesWord = "minute"

    let hours = parseInt(time/60/60%24), hoursWord;
    (hours > 1) || (hours === 0) ? hoursWord = "hours" : hoursWord = "hour"

    let days = parseInt(time/60/60/24), daysWord;
